...
type PromClient interface {
	QueryClusterMetrics(NodeMap map[string]int32) ([][]float64, error)
...
//  获取整个集群中的性能指标
func (p *promClient) QueryClusterMetrics(NodeMap map[string]int32) ([][]float64, error) {
	metrics := make([][]float64, len(NodeMap))
	count := 0
	for nodename := range NodeMap {
		nodeMetrice, err := p.QueryNodeMetrics(nodename)
		if err != nil {
			panic(err)
			//return nil, err
		}
